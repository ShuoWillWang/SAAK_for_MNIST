...
def convolution(train, test, train_label, test_label, k, stage, per=None):
	...
	sample, sample_label = generate_sample(train, train_label, 50000, 10)
	...
	# don't use kmeans_resample: set k = -1
	...
	pca.fit(sample_window - mean)
        #pca = KernelPCA(n_components = d,)
	#pca.fit(sample_window - mean)
	print pca.explained_variance_ratio_[:50] 
	if stage == 1: f_num = d-1
	else:
	...
	#save pca for each stage with all components
	#np.save('/data/orcs/yueru/Saak/PCA/lossy_cent_'+str(stage) + '_v'+str(version)+'.npy',pca.components_[:f_num,:])
...
def main():
    ...
    for k in [-1, -1, -1, -1, -1]:
        	...
        	np.save('/data/orcs/yueru/Saak/feature/train_before_f_test_'+str(stage)+'_v'+str(version)+'.npy', train)
        	np.save('/data/orcs/yueru/Saak/feature/test_before_f_test_'+str(stage)+'_v'+str(version)+'.npy', test)
        	stage += 1
